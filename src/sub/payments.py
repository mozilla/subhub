...
def delete_customer(uid: str) -> FlaskResponse:
    ...
    logger.debug(f"Enter {delete_customer.__name__} ({uid})")
    ...
    if deleted_payment_customer:
        ...
        response_code = 400
    logger.debug(f"Exit {delete_customer.__name__} ({uid})")
...
def add_user_to_deleted_users_record(
    ...
    logger.debug(
        f"Exit {add_user_to_deleted_users_record.__name__} ({user_id, cust_id, origin_system})",
        new_user=new_user,
    )
    return new_user
